...
function nextTurn()
{
	...
	if(turn == PLAYER2)
	{
		if(nextMove != -1)
		{
			ready = false;
			//move(nextMove);
			SendRequest( { url: "ReceiveJSON.html"
			, payload: JSON.stringify({getMove: true})
			, handler: function(response) { 
			    var opMove = JSON.parse(response).move;
			    alert(opMove); 
			    move(opMove);
			}
			});
	...
	if(turn == PLAYER1)
	{
		if(nextMove != -1)
		{
			ready = false;
			SendRequest( { url: "ReceiveJSON.html"
			, payload: JSON.stringify({getMove: false, move: nextMove})
			, handler: function(response) { 
